...
3. **Record cap** — the read stops at a configured record cap
   ...
   described above, so the worst case for such a read is one page plus
   `SEARCH_DENIED_PAGE_WALK` pages.
