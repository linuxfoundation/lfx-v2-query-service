...
| `access_check_query` | `keyword`, **or** `text` with a `keyword` subfield | The access-bucket walk aggregates on it. The searcher reads `GET /<index>/_mapping` on first use and picks `access_check_query` (keyword) or `access_check_query.keyword` (text + subfield), memoizing the answer and logging `resolved access key field` at `Info`. Any other shape falls back to `access_check_query.keyword` with a warning. A *failed* mapping call also falls back with a warning but is **not** memoized: it is retried after 30 seconds, so a transient error at boot cannot pin the wrong field and a persistent one does not cost a round-trip per request |
