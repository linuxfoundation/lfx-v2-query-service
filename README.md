...
- `SUMMARY_MAX_RECORDS`: Membership records `GET /query/memberships/summary` reads before it returns whole organization runs with `complete: false` and a `page_token` (1–50000, default: "5000"). The cap is checked after a whole page and yields to the denied-page walk while the caller has seen nothing. The read normally uses at most the larger of the cap rounded up to whole pages and one full page more than `SEARCH_DENIED_PAGE_WALK`; if no organization boundary exists yet, it continues to finish the first run. This extension is bounded by 50000 raw hits (the existing configurable cap ceiling, not a new setting): if neither a resumable boundary nor end-of-results is established within that ceiling, the read fails with `503` and no summaries, never a partial run. A `page_token` is accepted only from the same version of the summary read; a token from an earlier version is rejected with `400` and the read must restart without it (see the [summary contract](docs/query-service-contract.md#get-querymembershipssummary)).
